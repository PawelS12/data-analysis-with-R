...
```{r}
# Your code here:
numbers_single <- my_list["numbers"]
print(numbers_single) 
letters_double <- my_list[["letters_vec"]]
print(letters_double) 
mpg_column <- my_list$df_sample$mpg
print(mpg_column)
...
# my_list <- within(my_list, rm(letters_vec))
print(my_list)
```
...
```{r}
# Your code here:
element_lengths <- lapply(my_list, length)
print(element_lengths)
element_means <- sapply(my_list, function(x) if(is.numeric(x)) mean(x) else NA)
print(element_means)
```
...
log_list <- list(log_vec = log_vec)
nested_li <- list(my_list = my_list, log_list = log_list)
str(nested_li)
```
...
```{r}
# Your code here:
log_vec_access <- nested_li$log_list$log_vec
print(log_vec_access)
```
...
df_additional <- data.frame(Name = c("Alice", "Bob", "Charlie"), Age = c(25, 30, 35))
