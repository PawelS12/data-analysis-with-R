...
For more detail on this data set, consult Roger Peng’s book Statistical Methods in Environmental Epidemiology with R.
You can download the data we are using during this tutorial here.
...
#A Default ggplot First, to be able to use the functionality of {ggplot2} we have to load the package (which we can also load via the tidyverse package collection):
...
In accordance with the basic elements, a default ggplot needs three things that you have to specify: the `data`, `aesthetics`, and a `geometry.` We always start to define a plotting object by calling `ggplot(data = df)` which just tells `ggplot2` that we are going to work with that data.
...
There are many, many different geometries (called geoms because each function usually starts with `geom_`) one can add to a `ggplot` by default and even more provided by extension packages.
...
You can find more on how to use built-in themes and how to customize themes in the section “Working with Themes”.
...
Sometimes you want to zoom into take a closer look at some range of your data.
...
You may wonder: So in the end both result in the same.
But not really, there is an important difference—compare the two following plots:
...
\*\*The argument clip = "off" in any coordinate system, always starting with coord\_\*, allows to draw outside of the panel area.\*\*
...
All the following modifications of theme elements work not only for the title but for all other labels such as `plot.subtitle`, `plot.caption`, `legend.title`, `legend.text`, and `axis.title` and `axis.text.`
...
Or to phrase it in a more ggplot’ish way: we `map` the variable `season` to the `aesthetic` `color.` One nice thing about `ggplot2` is that it adds a legend by default when mapping a `variable` to an `aesthetic.` You can see that by default the legend title is what we specified in the color argument:
...
This is because there is a layer on top of the `panel.background`, namely `panel.border.` However, make sure to use a transparent fill here, otherwise your data is hidden behind this layer.
...
In this case the argument is `plot.margin.` As In the previous example we already illustrated the default margin by changing the background color using `plot.background.`
...
The easiest approach in my opinion is the `patchwork` package by Thomas Lin Pedersen:
...
For a more advanced treatment of the topic you should probably get your hands on Hadley’s book which has nice coverage.
Other good sources are the R Cookbook and the \`color section in the R Graph Gallery by Yan Holtz.
...
As you have already seen in the beginning of this tutorial, variables that are inside the aesthetics are encoded by variables and those that are outside are properties that are unrelated to the variables.
...
Qualitative Variables Qualitative or categorical variables represent types of data which can be divided into groups (categories).
...
You can pick your own set of colors and assign them to a categorical variables via the function \*\*scale\_\*\_manual()\*\* (the \* can be either color, colour, or fill).
...
The ColorBrewer palettes is a popular online tool for selecting color schemes for maps.
...
For an extensive overview of color palettes available in R, check the collection provided by Emil Hvitfeldt.
One can also use his `paletteer` package, a comprehensive collection of color palettes in R that uses a consistent syntax.

Examples:

The `ggthemes` package for example lets R users access the Tableau colors.
Tableau is a famous visualiztion software with a well-known color palette.
...
The `ggsci` package provides scientific journal and sci-fi themed color palettes.
...
Artwork by Allison Horst In our example we will change the variable we want to color to ozone, a continuous variable that is strongly related to temperature (higher temperature = higher ozone).
...
The viridis color palettes do not only make your plots look pretty and good to perceive but also easier to read by those with colorblindness and print well in gray scale.
...
Again, I point you to the great collection provided by Emil Hvitfeldt for an overview.
...
This seems a bit complicated for now—one could simply use the color and fill scales for both.
Yes, that is true but think about use cases where you need several color and/or fill scales.
In such a case, it would be senseless to occupy the fill scale with a slightly darker version of the palette used for color.
...
For a list you can visit the `ggthemes` package site.
...
Another example is the plotting style of Tufte, a minimal ink theme based on Edward Tufte’s book The Visual Display of Quantitative Information.
This is the book that popularized Minard’s chart depicting Napoleon’s march on Russia as one of the best statistical drawings ever created.
...
If you like the way of plotting have a look on this blog entry creating several Tufte plots in R.
...
If you have a closer look at the default theme you will notice that the sizes of all the elements are relative (`rel()`) to the base_size.
...
You can also use `geom_text()` if you don’t like boxes around your labels.
...
Have a look at all the usage examples.

## Add Text Annotations

There are several ways how one can add annotations to a ggplot.
We can again use `geom_text()` or `geom_label()`:
...
Again, we are using Claus Wilke’s `ggtext` package that is designed for improved text rendering support for `ggplot2`.
...
Note that this will only work for continuous data.
If you want to reverse discrete data, use the `fct_rev()` function from the `forcats` package.

## Transform an Axis

… or transform the default linear mapping by using `scale_y_log10()` or `scale_y_sqrt()`.
As an example, here is a log10-transformed axis (which introduces NA’s in this case so be careful):
...
We are going to plot the thresholds in dewpoint (i.e. the temperature at which airborne water vapor will condense to form liquid dew) related to temperature and ozone levels:
...
One can fix that by mapping also color to either ..count..
(the default) or ..density..…
...
While you can create those plots with basic `ggplot2` commands the popularity lead to a package that make it easier create those plots: `ggridges`.
...
The package also comes with its own theme .
...
This follows the idea of Marc Belzunces.
...
Default: Adding a LOESS or GAM Smoothing You can simply use `stat_smooth()`— not even a formula is required.
...
Maybe you want to use a polynomial regression?
...
Huh, `geom_smooth()`?
There is an important difference between geom and stat layers but here it really doesn’t matter which one you use.
Expand to compare both.
